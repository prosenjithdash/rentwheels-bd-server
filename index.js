...
async function run() {
  try {
    ...
    console.log(
      "Pinged your deployment. You successfully RentWheels-BD connected to MongoDB!"
    );
    ...
    const verifyRender = async (req, res, next) => {
      ...
      if (!result || result?.role !== "render") {
        ...
        return res
          .status(403)
          .send({ message: "Forbidden: Render access only" });
      }
      next();
    };
    ...
    app.get("/my_listings/:email", verifyToken, async (req, res) => {
      try {
        ...
        if (!result || result.length === 0) {
          return res
            .status(404)
            .send({ message: "No vehicles found for this host" });
        }
